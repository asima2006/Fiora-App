...
function InviteMessage(props: InviteMessageProps) {
    ...
    async function handleJoinGroup() {
        ...
        if (group) {
            ...
            // If user was auto-added to a community, add it to the state
            if (group.community) {
                group.community.type = 'community';
                action.addLinkman(group.community, false);
                Message.success(`Also joined community "${group.community.name}"`);
            }
        }
    }
        ...
        >
            <div className={Style.info}>
                <span className={Style.info}>
                    &quot;{invite.inviterName}&quot; invites you to join group 「
                    {invite.groupName}」
                </span>
            </div>
